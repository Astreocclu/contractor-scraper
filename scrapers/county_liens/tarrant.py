"""
Tarrant County Official Public Records scraper.
...
class TarrantCountyScraper(BaseCountyLienScraper):
    """
    Scraper for Tarrant County Official Public Records.
    
    Portal uses an Angular-based SPA for searching records.
    Supports searching by grantee name (the debtor/contractor).
    
    Note: This implementation is based on typical county portal patterns.
    Actual selectors may need adjustment after live testing.
    """
    
    COUNTY_NAME = "tarrant"
    BASE_URL = "https://tarrant.tx.publicsearch.us/"
    SEARCH_URL = "https://tarrant.tx.publicsearch.us/"
    
    # Document type codes used by Tarrant County portal
    # These may need verification against actual portal
    DOCUMENT_TYPE_CODES = {
        'MECH_LIEN': ['MECH LIEN', 'ML'],
        'REL_LIEN': ['REL LIEN', 'REL ML', 'RELEASE LIEN'],
        'ABS_JUDG': ['ABS JUDG', 'AJ', 'ABSTRACT JUDGMENT'],
        'FED_TAX_LIEN': ['FED TAX LIEN', 'FTL'],
        'STATE_TAX_LIEN': ['STATE TAX LIEN', 'STL', 'ST TAX LIEN'],
    }
    
    async def search_by_name(self, name: str) -> list[LienRecord]:
        """
        Search Tarrant County records by grantee (debtor) name.
        ...
        """
        logger.info(f"Searching Tarrant County for: {name}")
        ...
        try:
            ...
            await asyncio.sleep(1.0)  # Wait for Angular to initialize
            
            # Check for actual CAPTCHA challenge (not just config strings)
            # Look for visible reCAPTCHA iframe or challenge elements
            captcha_visible = await page.query_selector('iframe[src*="recaptcha"], .g-recaptcha, #captcha, [class*="captcha-challenge"]')
            if captcha_visible:
                raise CaptchaDetected("CAPTCHA challenge detected on Tarrant County portal")
            ...
            if search_input:
                ...
                raise CountyPortalUnavailable("Could not find search input on Tarrant portal")
            
            # Note: Date range is typically pre-set in the portal, skip manual date entry

            # Submit search - click the search button (magnifying glass icon)
            ...
            await asyncio.sleep(2.0)  # Give time for AJAX
            ...
            while page_num < max_pages:
                next_button = await page.query_selector('button:has-text("Next"), a:has-text("Next"), .pagination-next, [aria-label="Next"]')
            ...
            logger.info(f"Found {len(records)} records in Tarrant County for {name}")
            return records
            
        except PlaywrightTimeout as e:
            logger.error(f"Timeout on Tarrant County portal: {e}")
            raise CountyPortalUnavailable(f"Tarrant County portal timeout: {e}")
    ...
    async def search_by_date_range(
        ...
        """
        Search Tarrant County by date range.
        ...
        Returns:
            List of LienRecord
        """
        logger.info(f"Searching Tarrant County from {start} to {end}")
        ...
        try:
            ...
            # Fill date range
            start_input = await page.query_selector('input[name="startDate"], input#startDate')
            end_input = await page.query_selector('input[name="endDate"], input#endDate')
            ...
            # Select document types if supported
            # Implementation depends on portal UI
...
# CLI testing
async def main():
    """Test the scraper from command line."""
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python -m scrapers.county_liens.tarrant <business_name>")
        sys.exit(1)
