#!/usr/bin/env node
/**
 * RICHARDSON PERMIT SCRAPER
 * Portal: Accela Citizen Access
 * URL: https://aca-prod.accela.com/COR/Default.aspx
 *
...
const BASE_URL = 'https://aca-prod.accela.com/COR';
...
async function scrape(targetCount = 50) {
  ...
  console.log('RICHARDSON PERMIT SCRAPER (Accela)');
  console.log('='.repeat(50));
  ...
  try {
    ...
    console.log('[1] Loading Richardson Accela portal...');
    await page.goto(`${BASE_URL}/Cap/CapHome.aspx?module=Building&TabName=Building`, {
    ...
    while (permits.length < targetCount) {
      ...
      fs.writeFileSync(`debug_html/richardson_p${pageNum}.html`, await page.content());

      const extractPrompt = `Extract permit records from this Richardson Accela search results page.
...
${html.substring(0, 100000)}`;
  ...
  fs.writeFileSync('richardson_raw.json', JSON.stringify(output, null, 2));
  ...
  console.log(`Output: richardson_raw.json`);
