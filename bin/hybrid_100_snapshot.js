#!/usr/bin/env node
/**
 * Snapshot existing raw data for the 100-sample hybrid run.
 *
 * Usage:
 *   node bin/hybrid_100_snapshot.js [--group=A|B] [--config=path.json]
 */
...
console.log(`Snapshotting Group ${group} from ${CONFIG_PATH}`);
...
async function main() {
  ...
  for (const contractor of contractors) {
    try {
      ...
      if (rows.length === 0) {
        results.push({ id: contractor.id, name: contractor.name || contractor.business_name, status: 'NOT_FOUND' });
        continue;
      }
      ...
      if (rawData.length === 0) {
        results.push({ id: contractor.id, name: contractor.name || contractor.business_name, status: 'NO_DATA' });
        continue;
      }
      ...
      const safeName = (contractor.name || contractor.business_name || contractor.business_name || String(contractor.id)).toLowerCase().replace(/[^a-z0-9]/g, '_');
      const snapshotFile = path.join(snapshotPath, `${contractor.id}_${safeName}.json`);
      fs.writeFileSync(snapshotFile, JSON.stringify(snapshot, null, 2));

      results.push({ id: contractor.id, name: contractor.name || contractor.business_name, status: 'OK', sources: Object.keys(snapshot.sources).length });
    } catch (err) {
      results.push({ id: contractor.id, name: contractor.name || contractor.business_name, status: 'ERROR', error: err.message });
    }
  }
