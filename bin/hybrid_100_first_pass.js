#!/usr/bin/env node
/**
 * First pass holistic LLM scoring for 100-sample hybrid run.
 *
 * Usage:
 *   node bin/hybrid_100_first_pass.js [--group=A|B]
 */
...
console.log(`Running first pass for Group ${group} (${experimentDir})`);
...
async function main() {
  ...
  const partialPath = path.join(RESULTS_DIR, 'first_pass.partial.json');

  for (let index = 0; index < snapshots.length; index++) {
    const snapshot = snapshots[index];
    process.stdout.write(`Scoring ${index + 1}/${total}: ${snapshot.business_name}\n`);
